...
- Full-width red pill submit button; inline status message (green success / red error) for validation and submission errors.
- **Submission:** [Web3Forms](https://web3forms.com) (`https://api.web3forms.com/submit`), posted via `fetch` with `Accept: application/json` rather than a native form POST, so the page can show the confirmation modal (below) instead of navigating away. The access key lives as a hidden `access_key` input inside the form. `FormData` is built straight from the real `<form>` DOM element rather than hand-picked from component state — that's what carries the hCaptcha response token through automatically (see below) without the submit handler needing to know it exists.
- **Spam protection:** an hCaptcha widget (`<div class="h-captcha" data-captcha="true">` + the `https://web3forms.com/client/script.js` loader, placed before `</body>`) sits between the last field and the submit button. It's an unconditionally-rendered, always-empty-in-markup `<div>` — the external script injects the actual widget into it at runtime, and because the div's own template never changes, React re-renders (e.g. on every keystroke elsewhere in the form) don't touch or wipe those injected children.
- On a successful submit: form fields clear and the confirmation modal opens. On a server-reported failure or a network error: an inline status message appears (red, matching the existing validation-error styling) and the modal does not open — never treat a submit as successful client-side before the API confirms it.

**Confirmation modal** (Contact page, and the pattern to reuse for any future "thanks" / alert popup): a `position:fixed;inset:0` backdrop (`rgba(20,23,28,.6)`, `backdrop-filter:blur(2px)`, `z-index:100` — above the sticky header's `50` and its dropdown's `60`) centers a white `22px`-radius card (`box-shadow:0 40px 90px rgba(0,0,0,.4)`, same as the form card). Headline is Source Serif 4 700 (`26px/1.2`); body copy is Source Sans 3 in muted `#5A6270`; a single red pill button closes it. Dismiss via the button, a backdrop click (the card itself calls `stopPropagation` so clicks inside it don't bubble to the backdrop), or `Escape` — the same three dismissal paths already established by the nav's Resources/Partners dropdown.
